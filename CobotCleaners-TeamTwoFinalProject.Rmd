...
AvailableF <- robotData[-6,c(1,4)] #remove 1 mystery row at the end of the matrix
Available <- as.matrix(AvailableF[,-1])
Profit <- as.matrix(packageData[6,-1])

NPkgs <- ncol(Resources)
NRobot <- nrow(AvailableF)
...
$$R_{i,j}$$ -- number of resources 


Algebraicly that should look like this:

$$
 \begin{split}
 \begin{aligned}
    \text{Maximize  }   &  \sum_{i=1}^{NPkgs} P_i x_i \\
    \text{subject to } & \sum_{j=1}^{NRobot} R_{i,j}x_i \leq A_j \; \forall \; j\\
                       & x_i \geq 0  \; \forall \; i
  \end{aligned}
  \end{split}
  (\#eq:Algebraic Model)
...
```{r}
prodmodel <- MIPModel() %>%
  add_variable (x[i], i=1:NPkgs, type="continuous", lb=0) %>% #using integer so we don't clean partial houses.
  set_objective (sum_expr(Profit[i] * x[i] , i=1:NPkgs ), "max") %>%
  add_constraint (sum_expr(Resources[i,j] * x[j], j=1:NPkgs) # Left hand side of constraint
                  <= Available[i],    # Inequality and Right side of constraint
                  i=1:NRobot) %>% # Repeat for each resource, j.  
...
Available <- matrix(c(25, 35, 45, 30,36), 
              ncol=1,dimnames=c(ResNames,"Available"))
...
prodmodel2 <- MIPModel() %>%
  ...
  set_objective (sum_expr(Profit[i] * x[i] , i=1:NPkgs ), "max") %>%
  add_constraint (sum_expr(Resources[j,i]*x[i], i=1:NPkgs) # Left hand side of constraint
...
TransportationMod <- MIPModel() %>%
  add_variable(x[i,j], i=1:4, j=1:4, type = "binary", lb=0) %>% 
  set_objective(sum_expr(TransCost[i,j] * x[i,j], j=1:3, i=1:3)  , "min") %>%
    add_constraint (sum_expr(x[i,j]*(ElectDmd[j]+TransCost[i,j]), i=1:3)
                    <= BattSupp[i],
                    j=1:3) %>% #adds constraint for supply side limitation setup by the conditions of the problem. This is taken into account by using the == in the equality setup. 
    add_constraint (sum_expr(x[i,j], i=1:nSuppliers)
                    <= Capacity[j],
                    j=1:nCustomers) #adds constraint for capacity of customers, which while not limited, still needs to be considered as a constraint for the model. That's why it is <= equality.
```
